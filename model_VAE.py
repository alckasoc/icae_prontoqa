    ...
    def forward(
        ...
        for segment_idx in range(num_segments):
           # print(f"===============Segment {segment_idx}=======================")
            ...
            mean_vae  = self.mean(segment_compress_outputs)
            log_var_vae  = self.log_var(segment_compress_outputs)

            segment_compress_outputs = self.reparameterize(mean_vae, log_var_vae)
            ...
            compress_outputs[:, segment_idx*self.mem_size: self.mem_size*(segment_idx+1)] = segment_compress_outputs[mem_flag].view(batch_size, self.mem_size, -1)
